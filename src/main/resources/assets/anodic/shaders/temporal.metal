...
struct Params { float4x4 reprojection; float2 jitterUV; float historyWeight; uint valid; };
...
fragment half4 taa_fs(Out in [[stage_in]], texture2d<half> current [[texture(0)]],
    ...
    float d=depth.sample(nearestEdge,at);
    // Metallum flips vertex Y: native texture UV is (original NDC.xy+1)/2.
    float4 previous=p.reprojection*float4(in.uv*2-1,d,1);
    float2 oldUV=previous.xy/max(previous.w,1e-8)*.5+.5;
    float expected=previous.z/max(previous.w,1e-8);
    float weight=p.historyWeight;
    if (p.valid==0 || d<=0 || previous.w<=0 || any(oldUV<px*.5) || any(oldUV>1-px*.5) || expected<0 || expected>1) return half4(now,half(d));
    half4 old=half4(half3(reconstruct(history,oldUV)),0);
    float oldDepth=float(history.sample(nearestEdge,oldUV).a);
    if (abs(oldDepth-expected)>max(0.00001,abs(expected)*.02)) return half4(now,half(d));
    ...
    weight=min(weight,mix(.9,.65,clamp(motion/24,0.0,1.0)));
